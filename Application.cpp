...
const std::string VIKING_ROOM_MODEL_PATH = "resources/viking-room/viking_room.obj";
const std::string VIKING_ROOM_TEXTURE_PATH = "resources/viking-room/viking_room.png";

const std::string ICON_PATH = "resources/icon.png";
const std::string CUBE_TEXTURE = "resources/image.png";
const std::string CUBE_PATH = "resources/kube.obj";


const float cameraSpeed = 2.0f;
const float fastCamera = 6.0f;
const float sensitivity = 0.1f;
const float cubeSpin = 10.0f;
...
namespace glm {
    vec3 matrixRight(mat4 matrix) {
        return vec3(matrix[0][0], matrix[1][0], matrix[2][0]);
    }

    vec3 matrixUp(mat3 matrix) {
        return vec3(matrix[0][1], matrix[1][1], matrix[2][1]);
    }

    vec3 matrixForward(mat3 matrix) {
        return vec3(matrix[0][2], matrix[1][2], matrix[2][2]);
    }

    vec3 matrixPos(mat3 matrix) {
...
void Application::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "Da Volkan", nullptr, nullptr);
...
void Application::mouseMove() {
    ...
    if (firstMouse) {
        mouseDelta = glm::vec2(xpos, ypos);
        firstMouse = false;
    }

    float xoffset = xpos - mouseDelta.x;
    float yoffset = ypos - mouseDelta.y; 
    mouseDelta = glm::vec2(xpos, ypos);

    xoffset *= sensitivity * deltaTime;
    yoffset *= sensitivity * deltaTime;

    rotateCamera(xoffset, glm::vec3(0.0f, 0.0f, 1.0f));
    rotateCamera(yoffset, glm::matrixRight(cameraMatrix));
}


void Application::camMove() {
    // Camera movement
    glm::vec3 moveVector(0.0f, 0.0f, 0.0f);
    glm::vec3 forward = glm::matrixForward(cameraMatrix);
    glm::vec3 right = glm::matrixRight(cameraMatrix);
    ...
    if (std::abs(moveVector.x) > 0.1f || std::abs(moveVector.y) > 0.1f || std::abs(moveVector.z) > 0.1f) {
        bool shiftPress = glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS;
        moveVector = glm::normalize(moveVector) * deltaTime * (shiftPress ? fastCamera : cameraSpeed);
...
void Application::mainLoop() {
    while (isRunning) {
        ...
        modelMatrix = glm::rotate(modelMatrix, deltaTime * glm::radians(cubeSpin), glm::vec3(0.0f, 0.0f, 1.0f));
        drawFrame();
    }
...
void Application::rotateCamera(float amount, glm::vec3 axis) {
    cameraMatrix = glm::rotate(cameraMatrix, amount, axis);
}


void Application::moveCamera(glm::vec3 moveVector) {
    cameraMatrix = glm::translate(cameraMatrix, moveVector);
}
...
void Application::loadModel() {
    ...
    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, CUBE_PATH.c_str())) {
    ...
    for (const auto& shape : shapes) {
        for (const auto& index : shape.mesh.indices) {
            ...
            vertex.pos = {
                // An array of floats, so multiply index by 3 and get all 3 values
...
void Application::createTextureImage() {
    ...
    stbi_uc* pixels = stbi_load(CUBE_TEXTURE.c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
void Application::updateUniformBuffers(uint32_t currentImage) {
    ...
    // Set near and far clip planes at end of proj matrix
